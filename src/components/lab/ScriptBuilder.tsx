...
function FlowNode({ id, data, selected }: NodeProps) {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute -top-3.5 right-1 z-10 hidden gap-1 group-hover:flex">
        {!isStart && (
          <button
            ...
            className="nodrag nopan pointer-events-auto flex h-5 w-5 items-center justify-center rounded border border-[var(--border-2)] bg-[var(--bg-elevated)] text-[var(--text-2)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-1)]"
          >
        ...
        {!isStart && (
          <button
            ...
            className={`nodrag nopan pointer-events-auto flex h-5 w-5 items-center justify-center rounded border bg-[var(--bg-elevated)] hover:bg-[var(--bg-hover)] ${
              nodeDisabled ? "border-amber-500/60 text-amber-300" : "border-[var(--border-2)] text-[var(--text-2)] hover:text-[var(--text-1)]"
            }`}
          >
            <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <button
          ...
          className="nodrag nopan pointer-events-auto flex h-5 w-5 items-center justify-center rounded border border-[var(--border-2)] bg-[var(--bg-elevated)] text-[var(--text-2)] hover:bg-[var(--bg-hover)] hover:text-rose-400"
        >
          <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <p className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-[var(--text-2)]">
        {meta.label}
        {nodeDisabled && <span className="rounded-full bg-[var(--bg-elevated)] px-1.5 py-px text-[9px] font-semibold normal-case text-[var(--text-3)]">inactive</span>}
      </p>
      <p className="truncate text-sm font-medium text-[var(--text-1)]">{d.label || meta.label}</p>
      {d.subtitle && <p className="mt-0.5 truncate text-[11px] text-[var(--text-3)]">{d.subtitle}</p>}
      {d.note && <p className="mt-0.5 line-clamp-2 text-[10px] italic text-[var(--text-3)]">{d.note}</p>}
      {/* Call Goal: the must-cover checklist, numbered = priority order. */}
      {isCallGoal && (
        <ol className="mt-1 space-y-0.5">
          {stmts.length === 0 && <li className="text-[10px] italic text-[var(--text-3)]">No goals yet — add statements the call must cover.</li>}
          {stmts.map((s, i) => (
            <li key={i} className="flex gap-1.5 text-[11px] text-[var(--text-1)]" title={s}>
      ...
      {handles.map((h, i) => {
        ...
        return (
          <span key={h.id}>
            ...
            {h.label && (
              <span
                className={`absolute bottom-[18px] max-w-[72px] -translate-x-1/2 truncate text-[8px] font-semibold text-[var(--text-2)] ${
        ...
        <div className="absolute right-full top-1.5 mr-0 w-44 space-y-1">
          {stmts.map((s, i) => (
            <div key={i} className="flex items-center justify-end" title={s}>
              <span className="max-w-full truncate rounded-full border border-dashed border-[var(--border-2)] bg-[var(--bg-elevated)]/95 px-2 py-0.5 text-[9px] leading-tight text-[var(--text-2)]">
                + {snip(s, 34)}
              </span>
              <span className="h-px w-2.5 shrink-0 bg-[var(--bg-elevated)]" />
            </div>
          ))}
        </div>
      )}
      {canAddConnector && (
        <>
          ...
          />
          <button
            className="nodrag nopan absolute -bottom-3 -right-3 flex h-6 w-6 items-center justify-center rounded-full border border-[var(--border-2)] bg-[var(--bg-elevated)] text-sm font-bold leading-none text-[var(--text-2)] shadow transition hover:border-emerald-400 hover:bg-[var(--bg-hover)] hover:text-emerald-300"
      ...
      {label ? (
        <EdgeLabelRenderer>
          <div
            ...
            className="rounded border border-[var(--border)] bg-[var(--bg-card)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--text-2)]"
          >
            {String(label)}
          </div>
        </EdgeLabelRenderer>
...
function RunRecording({ callId }: { callId: string }) {
  ...
  if (unavailable) {
    return <p className="border-t border-[var(--border)]/80 px-3 py-1.5 text-[10px] text-[var(--text-3)]">Recording unavailable</p>;
  }
  return (
    <div className="flex items-center gap-2 border-t border-[var(--border)]/80 px-3 py-2" onClick={(e) => e.stopPropagation()}>
...
export default function ScriptBuilder({ onClose, initialScriptId }: Props) {
  ...
  const PALETTE: { payload: string; label: string; cls: string }[] = [
    ...
    { payload: "scenario", label: "Scenario", cls: "border-primary bg-primary/10" },
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-[var(--bg-app)]">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-[var(--border)] px-4 py-2.5">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          <label className="flex items-center gap-2 text-xs text-[var(--text-3)]" title="Use this script for test calls">
            <span>Active</span>
            <button
              type="button"
              ...
              className={`relative h-5 w-9 shrink-0 rounded-full transition disabled:opacity-40 ${
                activeScriptId === scriptId ? "bg-emerald-600" : "bg-[var(--bg-elevated)]"
              }`}
            >
          ...
          <button
            ...
            className="rounded-lg border border-[var(--border)] p-2 text-[var(--text-2)] transition hover:bg-[var(--bg-hover)] disabled:opacity-40"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
            ...
            className="rounded-lg border border-[var(--border)] p-2 text-[var(--text-2)] transition hover:bg-[var(--bg-hover)] hover:text-yellow-300"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
            ...
            className="rounded-lg border border-[var(--border)] p-2 text-[var(--text-2)] transition hover:bg-[var(--bg-hover)]"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {/* Duplicate workflow */}
          <button onClick={handleDuplicateScript} disabled={!scriptId || busy} title="Duplicate workflow — copy every box and arrow into a new script" className="rounded-lg border border-[var(--border)] p-2 text-[var(--text-2)] transition hover:bg-[var(--bg-hover)] disabled:opacity-40">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          {/* Close */}
          <button onClick={() => confirmDiscard() && onClose()} title="Close" className="rounded-lg border border-[var(--border)] p-2 text-[var(--text-2)] transition hover:bg-[var(--bg-hover)]">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        {!paletteOpen ? (
          <button
            ...
            className="flex w-7 shrink-0 flex-col items-center gap-2 border-r border-[var(--border)] py-3 text-[var(--text-3)] transition hover:bg-[var(--bg-card)] hover:text-[var(--text-2)]"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto p-3">
            {PALETTE.map((b) => (
              <button
                ...
                className={`flex w-full cursor-grab items-center gap-1.5 rounded-lg border-2 px-2.5 py-1.5 text-left text-xs font-medium text-[var(--text-1)] hover:brightness-125 active:cursor-grabbing disabled:opacity-40 ${b.cls}`}
              >
                {b.label}
              </button>
            ))}
          </div>
          <p className="shrink-0 border-t border-[var(--border)] p-2 text-[10px] text-[var(--text-3)]">
        ...
        <div className="flex min-w-0 flex-1 flex-col" onDrop={onDrop} onDragOver={onDragOver}>
          ...
          {run.status !== "idle" &&
            (() => {
              ...
              const head = "flex shrink-0 items-center justify-between border-b border-[var(--border)] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--text-3)]";
              ...
              const copyBtn = (label: string, lines: string[]) => (
                <button
                  ...
                  className="shrink-0 rounded p-0.5 text-[var(--text-3)] transition hover:bg-[var(--bg-hover)] hover:text-[var(--text-2)]"
                >
                  <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path
                      strokeLinecap="round"
              ...
              return (
                <div className="order-2 flex shrink-0 flex-col border-t border-[var(--border)] bg-[var(--bg-app)]">
                  {/* Drag the top edge to resize the dock */}
                  {runPanelOpen && (
                    <div
                      onMouseDown={startDockDrag}
                      title="Drag to resize"
                      className="h-1.5 w-full shrink-0 cursor-ns-resize bg-[var(--bg-elevated)]/60 transition hover:bg-emerald-500/40"
                    />
                  )}
                  ...
                  <div className="flex items-center gap-3 px-4 py-2">
                    <span
                      className={`h-2.5 w-2.5 shrink-0 rounded-full ${
                        run.status === "live" ? "animate-pulse bg-emerald-400" : run.status === "connecting" ? "animate-pulse bg-amber-400" : "bg-[var(--bg-elevated)]"
                      }`}
                    />
                    <span className="shrink-0 text-xs font-semibold text-[var(--text-1)]">
                    ...
                    {!runPanelOpen && run.lastLine && (
                      <span className="min-w-0 truncate text-[11px] italic text-[var(--text-3)]">“{snip(run.lastLine, 80)}”</span>
                    )}
                    <span className="flex-1" />
                    <button
                      onClick={() => setDockFull((f) => !f)}
                      title={dockFull ? "Exit fullscreen" : "Fullscreen"}
                      className="shrink-0 rounded-md border border-[var(--border)] p-1 text-[var(--text-3)] transition hover:bg-[var(--bg-hover)] hover:text-[var(--text-1)]"
                    >
                      <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        {dockFull ? (
                    ...
                    {run.status === "ended" ? (
                      <button
                        ...
                        className="shrink-0 rounded-full border border-[var(--border-2)] px-2.5 py-0.5 text-[11px] text-[var(--text-2)] hover:bg-[var(--bg-hover)]"
                      >
                        Dismiss
                      </button>
                    ) : (
                  ...
                  {runPanelOpen && (
                    ...
                    >
                      <div className="flex min-h-0 min-w-0 flex-col">
                        ...
                        <div className={col}>
                          {transcript.length === 0 && <p className="text-[11px] text-[var(--text-3)]">Waiting for the first words…</p>}
                          {transcript
                            .slice()
                            .reverse()
                            .map((e) => (
                              <p key={e.id} className="text-[11px] leading-snug">
                                ...
                                <span className="text-[var(--text-2)]">{e.content}</span>
                                {replyTag(e) && <span className="text-[var(--text-3)]"> · {replyTag(e)}</span>}
                              </p>
                            ))}
                        </div>
                      </div>
                      <div className="flex min-h-0 min-w-0 flex-col">
                        ...
                        <div className={col}>
                          {listener.length === 0 && <p className="text-[11px] text-[var(--text-3)]">Classifications and lines land here…</p>}
                          {listener
                            .slice()
                            .reverse()
                            .map((e) => (
                              <p key={e.id} className="text-[11px] leading-snug">
                                {e.event_type === "classified" ? (
                                  ...
                                  <>
                                    ...
                                    <span className="text-[var(--text-3)]">{snip(e.content ?? "", 80)}</span>
                                  </>
                                ) : e.event_type === "error" ? (
                                  <>
                                  ...
                                  <>
                                    ...
                                    <span className="text-[var(--text-2)]">{snip(e.content ?? "", 110)}</span>
                                    {e.latency_ms != null && <span className="text-[var(--text-3)]"> · {(e.latency_ms / 1000).toFixed(1)}s</span>}
                                  </>
                                )}
                              </p>
                            ))}
                      ...
                      <div className="flex min-h-0 min-w-0 flex-col">
                        ...
                        <div className={col}>
                          ...
                          {observer.length === 0 && !waitingFor && (
                            <p className="text-[11px] text-[var(--text-3)]">The observer narrates every turn here — expectations, verdicts, advice…</p>
                          )}
                          {observer
                            .slice()
                            .reverse()
                            .map((e) => (
                              <p key={e.id} className="text-[11px] italic leading-snug text-[var(--text-3)]">
                                {observerText(e)}
                              </p>
                            ))}
                        </div>
        ...
        {/* Configuration drawer — the Lab's full settings, opened via the cog.
            With a script open it waits for the scripts list: the persona box
        ...
        {configOpen && (scriptId === null || scripts.length > 0) && (
          <div className="flex w-96 shrink-0 flex-col border-l border-[var(--border)] max-md:absolute max-md:inset-y-0 max-md:right-0 max-md:z-20 max-md:w-full max-md:bg-[var(--bg-app)] max-md:shadow-2xl">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-3">
              <LabConfigForm
                onAssistantChange={(id) => setLabAssistantId(id)}
                scriptId={scriptId}
                scriptPersona={scripts.find((s) => s.id === scriptId)?.persona ?? ""}
                onPersonaSaved={(p) =>
        ...
        {!configOpen && (sd || selEdge) && (
          <div className="w-72 shrink-0 space-y-3 overflow-y-auto border-l border-[var(--border)] p-3 max-md:absolute max-md:inset-y-0 max-md:right-0 max-md:z-20 max-md:w-[min(100%,20rem)] max-md:bg-[var(--bg-app)] max-md:shadow-2xl">
            ...
            {selNode && sd && (
              <>
                <div>
                  <label className="mb-1 block text-xs text-[var(--text-3)]">Label</label>
                ...
                {sd.kind === "start" && (
                  <>
                    <div>
                      <label className="mb-1 block text-xs text-[var(--text-3)]">Opening mode</label>
                      <select
                    ...
                    {((sd.config.mode as string) ?? "agent_first") === "agent_first" && (
                      <div>
                        <label className="mb-1 block text-xs text-[var(--text-3)]">
                          Opening line <span className="text-[var(--text-3)]">(this script&rsquo;s own)</span>
                        </label>
                        <textarea
                  ...
                  <>
                    {["scenario", "collection", "send_sms", "end", "transfer", "call_goal"].includes(content) &&
                      (() => {
                        ...
                        return (
                          <details className="rounded-lg border border-[var(--border)]" open={isGoal || stmts.length > 0}>
                            <summary className="cursor-pointer px-2.5 py-2 text-[11px] font-semibold uppercase tracking-wider text-[var(--text-3)] hover:text-[var(--text-2)]">
                            ...
                            {stmts.map((s, i) => (
                              <div key={i} className="mb-1.5 flex items-start gap-1.5">
                                ...
                                <button
                                  ...
                                  className="mt-1 shrink-0 text-sm text-[var(--text-3)] hover:text-rose-400"
                            ...
                            <button
                              ...
                              className="rounded-md border border-dashed border-[var(--border-2)] px-2 py-1 text-[11px] text-[var(--text-3)] transition hover:border-primary hover:text-primary"
                            >
                    ...
                    {content === "scenario" && draft && (
                      <>
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">
                            Description <span className="text-[var(--text-3)]">(when does this fit?)</span>
                          </label>
                          <textarea
                        ...
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">
                            What the agent says <span className="text-[var(--text-3)]">(write new, or pick an existing scenario)</span>
                          </label>
                          <select
                        ...
                        <details className="rounded-lg border border-[var(--border)]">
                          ...
                          <div className="space-y-3 p-2.5">
                            <div>
                              <label className="mb-1 block text-xs text-[var(--text-3)]">
                                Also consider <span className="text-[var(--text-3)]">(router picks best)</span>
                              </label>
                              {((sd.config.candidateScenarioIds as string[]) ?? []).map((cid) => (
                                <button
                                  ...
                                  className="mb-1 mr-1 inline-flex items-center gap-1 rounded-full bg-primary/15 px-2 py-0.5 text-[11px] text-primary hover:-translate-y-px/25"
                                >
                                  {scenarioName(cid) ?? "scenario"} <span className="text-primary">×</span>
                                </button>
                              ))}
                              <select
                                className={inputCls + " [color-scheme:dark]"}
                            ...
                            <div>
                              <label className="mb-1 block text-xs text-[var(--text-3)]">
                                Active tags at this step <span className="text-[var(--text-3)]">(blank = all)</span>
                              </label>
                                ...
                                {allTags.map((t) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`rounded-full px-2 py-0.5 text-[10px] font-medium transition ${on ? "bg-purple-500/25 text-purple-200" : "border border-[var(--border)] text-[var(--text-3)]"}`}
                                    >
                                      {t}
                                    </button>
                                  );
                    ...
                    {content === "collection" && (
                      <>
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">Collection <span className="text-[var(--text-3)]">(the reply picks a member)</span></label>
                          <select
                            className={inputCls + " [color-scheme:dark]"}
                        ...
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">
                            Replies in this collection <span className="text-[var(--text-3)]">(answered in place)</span>
                          </label>
                          {(() => {
                            const cid = (sd.config.collectionId as string) ?? "";
                            if (!cid) return <p className="text-[11px] text-[var(--text-3)]">Pick a collection to see its replies.</p>;
                            const ids = colMembers[cid];
                            if (!ids) return <p className="text-[11px] text-[var(--text-3)]">Loading…</p>;
                            ...
                            if (!members.length) return <p className="text-[11px] text-[var(--text-3)]">This collection is empty — add scenarios to it in the Playbook.</p>;
                            return (
                              <ul className="space-y-1">
                                {members.map((s) => (
                                  <li key={s.id} className={`rounded-md bg-[var(--bg-elevated)] px-2 py-1.5 ${s.enabled ? "" : "opacity-50"}`}>
                                    <p className="flex items-center gap-1.5 text-[11px] font-medium text-[var(--text-2)]">
                                      <span className="min-w-0 truncate">{s.name}</span>
                                      {!s.enabled && (
                                        <span className="shrink-0 rounded-full bg-[var(--bg-elevated)] px-1.5 py-px text-[9px] font-semibold text-[var(--text-3)]" title="Toggled off in the Playbook — won't be matched on calls">
                                          off
                                        </span>
                                      )}
                                    </p>
                                    {s.description && <p className="mt-0.5 text-[10px] text-[var(--text-3)]">{snip(s.description, 84)}</p>}
                                  </li>
                                ))}
                              </ul>
                            );
                        ...
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">
                            ...
                            <span className="text-[var(--text-3)]">(write a line, use a scenario, or fall back to a collection)</span>
                          </label>
                          <select
                            className={inputCls + " mb-1.5 [color-scheme:dark]"}
                            value={
                          ...
                          {(sd.config.elseCollectionId as string) ? (
                            <p className="rounded-md bg-[var(--bg-elevated)] p-1.5 text-[10px] text-[var(--text-3)]">
                              Unmatched replies get a second chance against{" "}
                              <span className="text-[var(--text-2)]">{collectionName(sd.config.elseCollectionId as string) ?? "that collection"}</span>{" "}
                              ...
                              <>
                                ...
                                {sd.scenarioId && (
                                  <button
                                    ...
                                    className="mt-1.5 text-[11px] text-[var(--text-3)] transition hover:text-rose-400"
                                  >
                        ...
                        <p className="rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] p-2 text-[10px] text-[var(--text-3)]">
                    ...
                    {content === "subworkflow" && (
                      <div>
                        <label className="mb-1 block text-xs text-[var(--text-3)]">Sub-workflow <span className="text-[var(--text-3)]">(another script)</span></label>
                        <select
                          className={inputCls + " [color-scheme:dark]"}
                        ...
                        <p className="mt-1 text-[10px] text-[var(--text-3)]">When it finishes it returns a result; branch the next arrow on that result. Double-click the box on the canvas to preview its flow.</p>
                      </div>
                    )}
                    ...
                    {content === "send_sms" && draft && (
                      <>
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">Confirmation line <span className="text-[var(--text-3)]">(spoken as the text is sent)</span></label>
                          <textarea
                            className={inputCls + " min-h-[80px] resize-y"}
                            value={draft.text}
                            onChange={(e) => patchDraft(selNode.id, draft, { text: e.target.value })}
                        ...
                        <details className="rounded-lg border border-[var(--border)]">
                          ...
                          <div className="p-2.5">
                            <label className="mb-1 block text-xs text-[var(--text-3)]">Reuse an existing line</label>
                            <select
                              className={inputCls + " [color-scheme:dark]"}
                              value={sd.scenarioId ?? ""}
                              onChange={(e) => pickScenario(selNode.id, e.target.value || null)}
                    ...
                    {content === "transfer" && draft && (
                      <>
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">Transfer to (phone number)</label>
                        ...
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">Line to say while transferring</label>
                          <textarea
                    ...
                    {content === "wait" && (
                      <>
                        <p className="rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] p-2 text-[11px] text-[var(--text-3)]">
                        ...
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">
                            Silence timeout <span className="text-[var(--text-3)]">(seconds of total quiet before the silence path fires)</span>
                          </label>
                          <input
                            type="number"
                            min={2}
                    ...
                    {content === "return" && (
                      <div>
                        <label className="mb-1 block text-xs text-[var(--text-3)]">Result <span className="text-[var(--text-3)]">(handed back to the parent workflow)</span></label>
                        ...
                        <p className="mt-1 text-[10px] text-[var(--text-3)]">The parent can branch its next arrow on this result. Use this (not End Call) as a sub-workflow&rsquo;s normal exit.</p>
                      </div>
                    )}
                    ...
                    {content === "end" && draft && (
                      <>
                        <div>
                          <label className="mb-1 block text-xs text-[var(--text-3)]">
                            Goodbye line <span className="text-[var(--text-3)]">(optional)</span>
                          </label>
                          <textarea
                            className={inputCls + " min-h-[80px] resize-y"}
                            value={draft.text}
              ...
              (() => {
                ...
                if (isConnectorHandle(h)) {
                  ...
                  if (isTimeout) {
                    ...
                    return (
                      <>
                        <p className="rounded-lg border border-sky-800 bg-sky-500/5 p-2 text-xs text-[var(--text-2)]">
                          Silence path
                        ...
                        <button
                          ...
                          className="w-full rounded-md border border-[var(--border)] px-2 py-1.5 text-xs text-[var(--text-3)] transition hover:border-rose-500 hover:text-rose-300"
                        >
                  ...
                  return (
                    <>
                      <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] p-2">
                        <input
                          type="checkbox"
                          checked={isAny}
                      ...
                      <>
                      <div>
                        <label className="mb-1 block text-xs text-[var(--text-3)]">When should the agent use this?</label>
                        <textarea
                          className={inputCls + " min-h-[80px] resize-y"}
                      ...
                      <div>
                        <label className="mb-1 block text-xs text-[var(--text-3)]">
                          Quick match words <span className="text-[var(--text-3)]">(optional — instant routing)</span>
                        </label>
                        <input
                          className={inputCls}
                          value={(() => {
                ...
                return (
                  <p className="rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] p-2 text-[11px] text-[var(--text-3)]">
                    ...
                    <br />
                    <span className="text-[var(--text-3)]">Use Delete above to remove it.</span>
                  </p>
                );
              })()}
          </div>
      ...
      {history !== null && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6" onClick={() => setHistory(null)}>
          ...
          <div
            ...
            className="relative flex max-h-[75vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-app)] shadow-2xl"
          >
            <div className="border-b border-[var(--border)] px-5 py-3">
              <p className="text-[10px] uppercase tracking-wider text-[var(--text-3)]">Run history</p>
              <p className="text-sm font-bold text-[var(--text-1)]">{scripts.find((s) => s.id === scriptId)?.name ?? "Script"}</p>
            </div>
            <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto px-5 py-3">
              {histBusy && <p className="py-4 text-center text-xs text-[var(--text-3)]">Loading…</p>}
              {!histBusy && history.length === 0 && (
                <p className="py-4 text-center text-xs text-[var(--text-3)]">No runs yet — hit ▶ Run to make the first one.</p>
              )}
              {history.map((r) => {
                const endedAt = (nodes.find((n) => n.id === r.current_node_id)?.data as NodeData | undefined)?.label;
                return (
                  <div key={r.call_id} className="rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)]">
                    <button
                      onClick={() => viewRun(r)}
                      className="flex w-full items-center justify-between gap-3 rounded-t-lg px-3 py-2 text-left transition hover:bg-[var(--bg-card)]"
                    >
                      <span className="min-w-0">
                        <span className="block text-xs font-medium text-[var(--text-1)]">
            ...
            <div className="flex justify-end border-t border-[var(--border)] px-5 py-3">
              <button onClick={() => setHistory(null)} className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--text-2)] hover:bg-[var(--bg-hover)]">
                Close
              </button>
            </div>
          </div>
      ...
      {genOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6" onClick={() => !genBusy && setGenOpen(false)}>
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
          <div onClick={(e) => e.stopPropagation()} className="relative flex max-h-[82vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-app)] shadow-2xl">
            <div className="border-b border-[var(--border)] px-5 py-3">
              <p className="text-[10px] uppercase tracking-wider text-[var(--text-3)]">Generate from goals</p>
              <p className="text-sm font-bold text-[var(--text-1)]">Draft a starter script</p>
              <p className="mt-0.5 text-[11px] text-[var(--text-3)]">Lists the goals; we draft a flow (opener → beats → Q&amp;A → close) + a Call Goal box, then validate it. A scaffold to review, not final.</p>
            </div>
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-5 py-4">
              <div>
                <label className="mb-1 block text-xs text-[var(--text-3)]">Brand / campaign</label>
                <input className={inputCls} value={genBrand} onChange={(e) => setGenBrand(e.target.value)} placeholder="e.g. Lucky Seven Casino" />
              </div>
              <div>
                <label className="mb-1 block text-xs text-[var(--text-3)]">Goals — what the call must cover (add as many as you need)</label>
                <div className="space-y-1.5">
                  {genGoals.map((g, i) => (
                    <div key={i} className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="shrink-0 px-1 text-sm text-[var(--text-3)] hover:text-rose-400"
                        title="Remove goal"
                      >
                        ×
                      </button>
                  ...
                  <button
                    ...
                    className="rounded-md border border-dashed border-[var(--border-2)] px-2 py-1 text-[11px] text-[var(--text-3)] transition hover:border-yellow-500 hover:text-yellow-300"
                  >
                    + Add goal
                  </button>
                </div>
              ...
              <div>
                <label className="mb-1 block text-xs text-[var(--text-3)]">Persona (optional)</label>
            ...
            <div className="flex justify-end gap-2 border-t border-[var(--border)] px-5 py-3">
              <button onClick={() => setGenOpen(false)} disabled={genBusy} className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--text-2)] hover:bg-[var(--bg-hover)] disabled:opacity-40">Cancel</button>
      ...
      {simBusy && !sim && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
          <div className="relative flex flex-col items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--bg-app)] px-8 py-7 shadow-2xl">
            <svg className="h-7 w-7 animate-spin text-yellow-400" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
            </svg>
            <p className="text-sm font-semibold text-[var(--text-1)]">Simulating calls…</p>
            <p className="text-[11px] text-[var(--text-3)]">Playing out 10 conversations against the script — no call, no credits.</p>
          </div>
        </div>
      )}
      ...
      {sim && (() => {
        ...
        const sev = (s?: string) => (s === "high" ? "border-rose-500/40 bg-rose-500/10 text-rose-200" : s === "low" ? "border-[var(--border-2)]/40 bg-[var(--bg-elevated)]/20 text-[var(--text-2)]" : "border-amber-500/40 bg-amber-500/10 text-amber-200");
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6" onClick={() => setSim(null)}>
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
          <div onClick={(e) => e.stopPropagation()} className="relative flex max-h-[86vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-app)] shadow-2xl">
            <div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-3">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-[var(--text-3)]">Script simulation — no call, no credits</p>
                <p className="text-sm font-bold text-[var(--text-1)]">
                  {ready ? <span className="text-emerald-400">✓ Looks good</span> : <span className="text-amber-400">Review needed</span>}
                  <span className="ml-2 font-normal text-[var(--text-3)]">{convos.length} simulated call{convos.length === 1 ? "" : "s"} · {sim.structural.goals.length} goal{sim.structural.goals.length === 1 ? "" : "s"}</span>
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={runSimulate} disabled={simBusy} className="rounded-lg border border-[var(--border)] px-2.5 py-1 text-[11px] text-[var(--text-2)] hover:bg-[var(--bg-hover)] disabled:opacity-40">{simBusy ? "Simulating…" : "Re-run"}</button>
              </div>
            </div>
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
                ...
                <div className="rounded-xl border border-yellow-600/40 bg-yellow-600/[0.06] p-3">
                  <div className="mb-2 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setSuggestions(null)} disabled={applyBusy} className="rounded-lg border border-[var(--border)] px-2.5 py-1 text-[11px] text-[var(--text-2)] hover:bg-[var(--bg-hover)] disabled:opacity-40">Dismiss</button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    {suggestions.map((s, si) => (
                      <div key={si} className="rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)]">
                        <div className="flex items-start justify-between gap-3 border-b border-[var(--border)] px-3 py-2">
                          <div className="min-w-0">
                            ...
                            {s.reason && <p className="mt-0.5 text-[11px] text-[var(--text-3)]">{s.reason}</p>}
                          </div>
                        ...
                        <div className="space-y-2 px-3 py-2">
                          {s.edits.map((e, k) => (
                            <div key={k} className="text-[11px]">
                              <p className="mb-0.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--text-3)]">{e.label}</p>
                  ...
                  <p className="mt-2 text-[10px] text-[var(--text-3)]">Applies to THIS script in place — no duplicate. After it applies you can test a call, then use “↶ Undo auto-fix” if it didn’t help.</p>
                </div>
              )}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={i} className={`rounded-lg border px-3 py-2 text-center ${s.good ? "border-emerald-500/30 bg-emerald-500/5" : "border-amber-500/30 bg-amber-500/5"}`}>
                      ...
                      <p className="text-[10px] text-[var(--text-3)]">{s.l}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {rep.length > 0 && (
                <div className="space-y-1.5">
                  <p className="text-[11px] font-semibold uppercase tracking-wider text-[var(--text-3)]">Repeated points in the script <span className="font-normal normal-case text-[var(--text-3)]">— same idea, even reworded</span></p>
                  {rep.map((r, i) => (
              ...
              {convos.length > 0 && (
                <div>
                  <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-[var(--text-3)]">Simulated conversations</p>
                  <div className="space-y-2">
                    {convos.map((c, i) => {
                      ...
                      return (
                        <div key={i} className="overflow-hidden rounded-lg border border-[var(--border)]">
                          <button onClick={() => setOpenConvo(open ? null : i)} className="flex w-full items-center gap-2 px-3 py-2 text-left hover:bg-[var(--bg-elevated)]">
                            <span className={`shrink-0 text-[var(--text-3)] transition ${open ? "rotate-90" : ""}`}>▸</span>
                            <span className="flex-1 text-[12px] font-semibold text-[var(--text-1)]">{c.persona}</span>
                            {c.failed ? (
                              ...
                              <>
                                ...
                                {!c.endedCleanly && <span className="rounded-full bg-[var(--bg-elevated)] px-2 py-px text-[9px] font-semibold text-[var(--text-2)]">cut off</span>}
                              </>
                            )}
                          </button>
                          {open && !c.failed && (
                            <div className="border-t border-[var(--border)] bg-[var(--bg-app)] px-3 py-3">
                              <p className="mb-2 text-[10px] italic text-[var(--text-3)]">Customer: {c.behavior}</p>
                              <div className="space-y-1.5">
                                {c.transcript.map((t, k) => (
                                  <div key={k} className={`flex ${t.speaker === "agent" ? "justify-start" : "justify-end"}`}>
                                    <div className={`max-w-[78%] rounded-2xl px-3 py-1.5 text-[11px] leading-snug ${t.speaker === "agent" ? "rounded-tl-sm bg-[var(--bg-elevated)] text-[var(--text-1)]" : "rounded-tr-sm bg-primary/80 text-white"}`}>
                                      <span className="mb-0.5 block text-[8px] font-semibold uppercase tracking-wider opacity-50">{t.speaker}</span>
                                      {t.text}
                                    </div>
                                  </div>
                                ))}
                                {c.transcript.length === 0 && <p className="text-[11px] text-[var(--text-3)]">No transcript returned.</p>}
                              </div>
                              ...
                              {c.notes && <p className="mt-2 text-[10px] text-[var(--text-3)]">{c.notes}</p>}
                            </div>
                          )}
                        </div>
                      );
              ...
              {sim.structural.issues.length > 0 && (
                <div className="space-y-1.5">
                  <p className="text-[11px] font-semibold uppercase tracking-wider text-[var(--text-3)]">Flow checks</p>
              ...
              <div>
                <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-[var(--text-3)]">Goal coverage <span className="font-normal normal-case text-[var(--text-3)]">(every possible path)</span></p>
                <div className="overflow-hidden rounded-lg border border-[var(--border)]">
                  {sim.structural.goals.length === 0 && <p className="px-3 py-2 text-[11px] text-[var(--text-3)]">No Call Goals — add a Call Goal box to check the offer is delivered.</p>}
                  {sim.structural.goals.map((g, i) => (
                    <div key={i} className={`flex items-start gap-2 px-3 py-2 text-[11px] ${i % 2 ? "bg-[var(--bg-elevated)]" : ""}`}>
                      <span className="shrink-0">{!g.everCovered ? "❌" : g.guaranteed ? "✅" : "⚠️"}</span>
                      <div className="min-w-0 flex-1">
                        <span className="text-[var(--text-1)]">{g.text}</span>
              ...
              {convos.length === 0 && !sim.reviewError && <p className="rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] px-3 py-2 text-[11px] text-[var(--text-3)]">Structural self-check only. Hit <span className="font-semibold text-[var(--text-1)]">Re-run</span> to play out 10 conversations against this script.</p>}
              ...
              <p className="text-[10px] text-[var(--text-3)]">Dry-run only — an AI plays both the agent (following your script) and the customer. It approximates a real call to preview coverage &amp; repetition; it is not the live engine. Save the script first if you edited scenario lines inline.</p>
            </div>
            <div className="flex justify-end border-t border-[var(--border)] px-5 py-3">
              <button onClick={() => setSim(null)} className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--text-2)] hover:bg-[var(--bg-hover)]">Close</button>
            </div>
          </div>
        </div>
        );
      ...
      {undoBuffer && (
        <div className="fixed left-1/2 top-4 z-[80] flex -translate-x-1/2 items-center gap-3 rounded-full border border-[var(--border)] bg-[var(--bg-app)]/95 px-4 py-2 shadow-2xl">
          <span className="text-[12px] text-[var(--text-1)]">Script was auto-fixed.</span>
          ...
          <button onClick={() => setUndoBuffer(null)} className="text-[14px] leading-none text-[var(--text-3)] hover:text-[var(--text-2)]" title="Dismiss (keep the fix)">×</button>
        </div>
      )}
      ...
      {rulesOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6" onClick={() => setRulesOpen(false)}>
          ...
          <div
            onClick={(e) => e.stopPropagation()}
            className="relative flex max-h-[82vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-app)] shadow-2xl"
          >
            <div className="border-b border-[var(--border)] px-5 py-3">
              <p className="text-[10px] uppercase tracking-wider text-[var(--text-3)]">Voice AI Call System</p>
              <p className="text-sm font-bold text-[var(--text-1)]">Rules &amp; Architecture</p>
            ...
            <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-4">
              {SYSTEM_RULES.map((section) => (
                <div key={section.title}>
                  <p className="text-xs font-bold uppercase tracking-wider text-primary">{section.title}</p>
                  <p className="mb-2 text-[11px] italic text-[var(--text-3)]">{section.subtitle}</p>
                  <div className="overflow-hidden rounded-lg border border-[var(--border)]">
                    {section.rules.map((r, i) => (
                      <div key={r.name} className={`flex gap-3 px-3 py-2 ${i % 2 ? "bg-[var(--bg-elevated)]" : ""}`}>
                        <div className="w-40 shrink-0">
                          <span className="text-[11px] font-semibold text-[var(--text-1)]">{r.name}</span>
                        ...
                        <span className="flex-1 text-[11px] leading-snug text-[var(--text-3)]">{r.spec}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="flex justify-end border-t border-[var(--border)] px-5 py-3">
              <button onClick={() => setRulesOpen(false)} className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--text-2)] hover:bg-[var(--bg-hover)]">
                Close
              </button>
            </div>
          </div>
      ...
      {qa && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6" onClick={() => setQa(null)}>
          ...
          <div
            onClick={(e) => e.stopPropagation()}
            className="relative flex max-h-[80vh] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-app)] shadow-2xl"
          >
            <div className="border-b border-[var(--border)] px-5 py-3">
            ...
            <div className="flex items-center justify-end gap-2 border-t border-[var(--border)] px-5 py-3">
              <button onClick={() => setQa(null)} className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--text-2)] hover:bg-[var(--bg-hover)]">
                Close
              </button>
              <button
                onClick={startRun}
      ...
      {preview && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6" onClick={() => setPreview(null)}>
          ...
          <div
            onClick={(e) => e.stopPropagation()}
            className="relative flex h-[80vh] w-[85vw] max-w-5xl flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-app)] shadow-2xl"
          >
            <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-2.5">
              <div className="min-w-0">
                <p className="text-[10px] uppercase tracking-wider text-[var(--text-3)]">Sub-workflow preview</p>
                <p className="truncate text-sm font-bold text-[var(--text-1)]">{preview.name}</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                  ...
                  className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--text-2)] hover:bg-[var(--bg-hover)]"
                >
                  Open for editing
                </button>
                <button onClick={() => setPreview(null)} className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--text-2)] hover:bg-[var(--bg-hover)]">
                  Close
                </button>
              </div>
            </div>
