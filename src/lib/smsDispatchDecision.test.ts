...
// ── optin_reached_only: an early hang-up who HEARD the offer (Jasiel 2026-09-10) ──────────
// Measured 11 Aug to 10 Sep on optin_reached_only campaigns: 1,048 early hang-ups, and on 351
// of them (33.5%) the agent had already said the offer ("I've added twenty free spins for you")
// before the player hung up. The other two thirds hung up on the greeting and never heard it.
// The rule: an early hang-up is texted ONLY when the agent spoke the offer. The offer link is the
// payload, and a player who heard "free spins" and hung up has something to receive; a player
// who hung up on "have you logged in recently?" has not. Everything the mode already refuses
// (voicemail, silent pickup, agent timeout, opt-out) stays refused whatever the agent said.
describe("decideSmsDispatch — optin_reached_only texts an early hang-up who heard the offer (2026-09-10)", () => {
  const base: SmsDispatchInput = {
    mode: "optin_reached_only",
    goalReached: false,
    nativeSuccess: false,
    ...
    customerDeclinedSms: false,
    humanConversation: false,
    attemptTag: "early_hangup",
  };

  it("early hang-up + offer spoken → text, with its OWN reason so logs can count the rule", () => {
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: true }))
      .toEqual({ attempt: true, reason: "early_hangup_offer_spoken" });
  });

  it("early hang-up, offer NOT spoken → no text (two thirds of early hang-ups: they hung up on the greeting)", () => {
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: false }))
      .toEqual({ attempt: false, reason: "early_hangup" });
  });

  it("FAIL SAFE: a caller that does not say whether the offer was spoken gets today's behaviour, no text", () => {
    // The Audience drawer's why-no-text and any older caller omit the field. Silence must read
    // as "not spoken", never as "spoken": the rule widens sends and must be opted INTO.
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: undefined }))
      .toEqual({ attempt: false, reason: "early_hangup" });
    expect(decideSmsDispatch(base)).toEqual({ attempt: false, reason: "early_hangup" });
  });

  it("the offer being spoken changes NOTHING for the other refused buckets", () => {
    // A monologue into dead air (silent pickup) is still zero human evidence; a pipeline death is
    // still a redial we owe, not a text; a voicemail is still a machine; an unreached line is unreached.
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: true, attemptTag: "silent_pickup" }))
      .toEqual({ attempt: false, reason: "silent_pickup" });
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: true, attemptTag: "agent_timeout" }))
      .toEqual({ attempt: false, reason: "agent_timeout" });
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: true, attemptTag: "voicemail" }))
      .toEqual({ attempt: false, reason: "voicemail" });
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: true, attemptTag: "unreachable" }).attempt).toBe(false);
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: true, attemptTag: undefined }).attempt).toBe(false);
  });

  it("the two absolute vetoes still beat a spoken offer", () => {
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: true, optedOut: true }))
      .toEqual({ attempt: false, reason: "opted_out_on_call" });
    expect(decideSmsDispatch({ ...base, agentSpokeOffer: true, voicemailDetected: true }))
      .toEqual({ attempt: false, reason: "voicemail" });
  });

  it("the other modes ignore the field entirely (their policies are unchanged)", () => {
    // KNOWN-BAD CONTROL for scope: flipping the flag must not move a single decision outside
    // optin_reached_only. If it does, the rule leaked.
    for (const mode of ["verbal_yes", "registered_optin", "optin_any_pickup"] as const) {
      for (const tag of ["early_hangup", "neutral", "silent_pickup", "voicemail"] as const) {
        const off = decideSmsDispatch({ ...base, mode, attemptTag: tag, agentSpokeOffer: false });
        const on = decideSmsDispatch({ ...base, mode, attemptTag: tag, agentSpokeOffer: true });
